...
class Quiz_controller extends CI_Controller {
	...
	*/
	...
	public function get_assessment_array_result() {

		$assessment_id_list = $this->input->get('assessment_id');
		$assessment_result = array();
		if ($assessment_id_list != null) {
			foreach ($assessment_id_list as $key => $assessment_id_value) {
				$result_array = $this->quiz_model->getAssessmentData($assessment_id_value);
				$result_array['bar_title'] = 'Assessment ' . ($key + 1);
				array_push($assessment_result, $result_array);
			}
		}

		echo json_encode($assessment_result);
